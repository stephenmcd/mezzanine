...
also control this behavior by implementing a :meth:`~mezzanine.pages.admin\
.PageAdmin.in_menu` method on your admin class, which should return ``True``
or ``False``. When implemented, this method controls whether the admin class
appears in the menu or not. Here's an advanced example that excludes the
:class:`.BlogCategoryAdmin` class from the menu, unless it is explicitly
defined in :ref:`ADMIN_MENU_ORDER`::
