...
.. note::
    ...
    The ``collectstatic`` command gathers static files from each
    component and places them in a new or existing "static" directory.
    These files are not meant to be edited, and will be overwritten
    if you run the command again after upgrading packages (which you
    should do). Therefore, the "static" directory is not committed to
    repositories and it is in the .gitignore file generated by the
