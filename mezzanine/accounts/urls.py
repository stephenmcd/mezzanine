...
urlpatterns = [
    url(r"^%s%s$" % (LOGIN_URL.strip("/"), _slash),
        views.login, name="login"),
    url(r"^%s%s$" % (LOGOUT_URL.strip("/"), _slash),
        views.logout, name="logout"),
    url(r"^%s%s$" % (SIGNUP_URL.strip("/"), _slash),
        views.signup, name="signup"),
    url(r"^%s%s%s$" % (SIGNUP_VERIFY_URL.strip("/"), _verify_pattern, _slash),
        views.signup_verify, name="signup_verify"),
    url(r"^%s%s$" % (PROFILE_UPDATE_URL.strip("/"), _slash),
        views.profile_update, name="profile_update"),
    url(r"^%s%s$" % (PASSWORD_RESET_URL.strip("/"), _slash),
