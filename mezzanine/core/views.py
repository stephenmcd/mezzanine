...
def static_proxy(request):
    ...
    url = request.GET["u"]
    urlpath = urlparse(unquote(url)).path
    ...
    for prefix in (static_url, "/"):
        if urlpath.startswith(prefix):
            urlpath = urlpath.replace(prefix, "", 1)
    response = ""
    mimetype = ""
    path = finders.find(urlpath)
    if path:
        if isinstance(path, (list, tuple)):
            path = path[0]
        if urlpath.endswith(".htm"):
            # Inject <base href="{{ STATIC_URL }}"> into TinyMCE
            # plugins, since the path static files in these won't be
            # on the same domain.
            static_url = settings.STATIC_URL + os.path.split(urlpath)[0] + "/"
