...
class CreateTask(AbstractDeployTask):
    ...
    def run(self):
        ...
        psql_task.run("CREATE DATABASE %s WITH OWNER %s ENCODING = 'UTF8' "
                      "LC_CTYPE = '%s' LC_COLLATE = '%s' TEMPLATE template0;" %
                      (self.env.proj_name, self.env.proj_name, self.env.locale, self.env.locale))
            ...
            with cd(conf_path):
                ...
                if not exists(crt_file) and not exists(key_file):
                    try:
                        ...
                        self.as_sudo("openssl req -new -x509 -nodes -out %s -keyout %s "
        ...
        with self.project():
            ...
            if self.env.reqs_path:
                pip_task.run("-r %s/%s" % (self.env.proj_path, self.env.reqs_path))
            pip_task.run("gunicorn setproctitle south psycopg2 "
                         "django-compressor python-memcached")
            ManageTask(self.env).run("createdb --noinput --nodata")
            python_task = PythonTask(self.env)
            python_task.run("from django.conf import settings;"
                            "from django.contrib.sites.models import Site;"
                            "Site.objects.filter(id=settings.SITE_ID).update(domain='%s');"
                            % self.env.domains[0])
            for domain in self.env.domains:
                PythonTask(self.env).run("from django.contrib.sites.models import Site;"
            ...
            if self.env.admin_pass:
                ...
                shadowed = "*" * len(pw)
                self.print_command(user_py.replace("'%s'" % pw, "'%s'" % shadowed))
